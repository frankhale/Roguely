...
namespace roguely::common
{
  ...
  struct Size
  {
    bool eq(Size s) { return s.width == width && s.height == height; }
  ...
  struct Dimension
  {
    bool eq(Dimension d) { return d.point.eq(point) && d.supplimental_point.eq(supplimental_point) && d.size.eq(size); }
...
namespace roguely::ecs
{
  ...
  public:
    ...
    std::shared_ptr<std::vector<std::shared_ptr<Entity>>> find_entities_in_group(const std::string &entity_group, std::function<bool(std::shared_ptr<Entity>)> predicate);

    std::shared_ptr<Entity> find_entity(const std::string &entity_group, std::function<bool(std::shared_ptr<Entity>)> predicate);
    ...
    sol::table get_lua_blocked_points(const std::string &entity_group, int x, int y, const std::string &direction, sol::this_state s);
...
namespace roguely::components
{
  ...
  {
  public:
    LuaComponent(const std::string &n, sol::table props, sol::this_state s)
        : Component(n)
    {
      sol::state_view lua(s);
    ...
    T get_property(const std::string &name)
    {
      if (properties[name].valid())
      {
        ...
        if (std::is_same<T, sol::object>::value)
        {
          return value;
        }
...
namespace roguely::map
{
  ...
  public:
    ...
    void draw_map(SDL_Renderer *renderer, roguely::common::Dimension dimensions, int x, int y, int a, std::function<void(int, int, int)> draw_hook);
  ...
  public:
    ...
    std::vector<std::pair<int, int>> FindPath(const boost::numeric::ublas::matrix<int> &grid, int start_row, int start_col, int goal_row, int goal_col);
...
namespace roguely::engine
{
  ...
  private:
    ...
    void draw_graphic(SDL_Renderer *renderer, const std::string &path, int window_width, int x, int y, bool centered, int scale_factor);
                                                      ...
                                                      roguely::common::Size initial_view_port)
    {
      ...
      auto dimensions = roguely::common::Dimension{view_port_x, view_port_y, player_position.x, player_position.y, view_port_width, view_port_height};
